    ...
    <section className="relative w-full pt-24 md:pt-48 pb-30 md:pb-45 bg-[#fdfaf5] overflow-hidden border-t border-b border-[#eee3d5]">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6">
        <div className="flex flex-col lg:flex-row justify-between items-center gap-16 md:gap-24">
          ...
          {/* listagem dinâmica: mapeia o array 'levels' para criar os cards de navegação */}
          <div className="flex flex-col gap-6 w-full lg:w-auto min-w-[320px] md:min-w-[450px]">
            {!isLoading &&
              levels.map((level) => (
                <Link
                  key={level.id}
                ...
                >
                  {/* card container: possui bordas arredondadas e efeito de hover no padding lateral esquerdo */}
                  <div className="bg-white/90 backdrop-blur-sm border border-[#eee3d5] border-l-[6px] border-l-[#3d5a5a] p-6 md:p-8 rounded-r-[1rem] rounded-l-[0.3rem] shadow-[0_8px_20px_-10px_rgba(0,0,0,0.08)] transition-all duration-500 group-hover:shadow-[0_20px_40px_-10px_rgba(202,73,82,0.15)] group-hover:-translate-y-1 group-hover:border-l-[#ca4952]">
                    {/* sistema de estrelas: gera um array dinâmico baseado no número de estrelas do nível */}
                    <div className="flex gap-1 mb-3 text-[#ca4952] opacity-40 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex flex-col">
                        {/* internacionalização: usa a labelkey do json para acessar o texto traduzido no dicionário 'texts' */}
                        <h3
                      ...
                      {/* seta indicativa: animada via transform e opacity ao passar o mouse sobre o card */}
                      <div className="text-[#ca4952] opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0 font-bold text-2xl">
                        ➔
                      </div>
                    </div>
                  </div>

                  {/* camada de destaque (overlay): surge no hover para reforçar o feedback visual */}
                  <div className="absolute inset-0 bg-[#ca4952]/5 rounded-r-[1rem] rounded-l-[0.3rem] scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-500 -z-10"></div>
                </Link>
              ))}
          </div>
        </div>
